...
void MatrixStack::shear(const glm::vec2& x, const glm::vec2& y, const glm::vec2& z) {
	shear(x.x, x.y, y.x, y.y, z.x, z.y);
}

void MatrixStack::shear(float xy, float xz, float yx, float yz, float zx, float zy) {
	glm::mat4 S(1.0f);
	S[0][1] = xy; // x shear in y direction
	S[0][2] = xz; // x shear in z direction
	S[1][0] = yx; // y shear in x direction
	S[1][2] = yz; // y shear in z direction
	S[2][0] = zx; // z shear in x direction
	S[2][1] = zy; // z shear in y direction
	multMatrix(S);
}
